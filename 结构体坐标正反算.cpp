...
void ZhenSuan(tagPnt2D p1,double *S, double *T)
{
	tagPnt2D p2;
	p2.x= p1.x + *S * cos(*T);
	printf("p2.x=%lf\n", p2.x);
...
void FanSuan(tagPnt2D p1, tagPnt2D p2)
{
	double S = 0;
	double T;
	double ang = 0;
...
int main()
{
	...
	struct tagPnt2D p1 = { x1, y1 };
	ZhenSuan( p1,&s, &T);
	printf("请输入已知两点的点坐标(即x1,y1，x2,y2):\n");
	scanf("%lf %lf %lf %lf", &x2, &y2, &x3, &y3);
	struct tagPnt2D p2 = { x2, y2 };
	struct tagPnt2D p3 = { x3, y3 };
	FanSuan(p2, p3);
	system("pause");
	return 0;
}
